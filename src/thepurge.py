...
def the_purge(folder_path: str = ".", extensions: set[str] = None, strategy: str = PRINT, processes: int = 1, destination_dir: str = None):
    ...
    if strategy == MOVE:
        if destination_dir is None or not os.path.exists(destination_dir) or not os.path.isdir(destination_dir):
    ...
    deduplicator = Deduplicator(dir.recursive_list(), strategy=strategy_func, processes=processes)
...
def main(
    ...
    destination_dir: Annotated[
        str,
        typer.Option(
            help="Destination directory used to move deplicated files"
        ),
    ] = None,
    processes: Annotated[
        int,
    ...
    the_purge(folder_path=folder_path, extensions=set(extensions), strategy=strategy.upper(), processes=processes, destination_dir=destination_dir)
